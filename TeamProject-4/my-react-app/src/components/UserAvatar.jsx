...
export default function UserAvatar({ username = 'login', email = 'email@example.com', img = imgAvatarDefault }) {
    ...
    return (
        <div className="relative">
            <div id="user-avatar" className="fixed top-4 right-4 flex items-center gap-3 cursor-pointer" onClick={toggleMenu}>
            ...
            {isMenuOpen && (
                <div ref={menuRef} className="absolute right-4 top-[3.2rem] mt-2 w-48 bg-white border border-gray-300 rounded-md shadow-lg">
                    <div className="px-4 py-2 text-gray-800">{username}</div>
                    <div className="px-4 py-2 text-gray-600">{email}</div>
                    <div className="border-t border-gray-300"></div>
                    <a href="/impostazioni" className="block px-4 py-2 text-blue-600 hover:bg-gray-100">
                        Impostazioni
                    </a>
